...
import { ContractBulls, ContractExceptions, ContractOfferProcess, InstantBookingJobPayload } from '../types';

@Processor(ContractBulls.CONTRACT_OFFER_QUEUE)
export class InstantBookingProcessor {
  constructor(
  ...
  @Process(ContractOfferProcess.INSTANT_BOOKING)
  async handle(job: Job<InstantBookingJobPayload>) {
    ...
    // step 3: send system message (do nothing if exception)
    try {
      ...
      const message = await this.commandBus.execute<PushSystemMessageCommand, Result<MessageEntity, Error>>(
        new PushSystemMessageCommand({
          ...
          content: {
            type: SystemMessageType.INSTANT_BOOKING,
  ...
  private async payContractOrFail(contractId: UUID) {
    const payResult = await this.commandBus.execute<
      ...
      Result<{ payedTransactionId: UUID; customerReference: string }, Error | InnopayServiceBadRequestProblem>
  ...
  private async findChatByContract(contractId: string) {
