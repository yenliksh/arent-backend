...
import { ContractBulls, ContractExceptions, ContractOfferProcess, TemporaryConcludeJobPayload } from '../types';

@Processor(ContractBulls.CONTRACT_OFFER_QUEUE)
export class ContractTemporaryConcludeProcessor {
  constructor(
  ...
  @Process(ContractOfferProcess.TEMPORARY_CONCLUDE)
  async handle(job: Job<TemporaryConcludeJobPayload>) {
    ...
    try {
      ...
      await Promise.all([
        ...
        temporaryPaymentTransactions.map((temporaryTransaction) =>
          this.temporaryPaymentTransactionRepository.save(temporaryTransaction, trxId),
        ),
  ...
  private async startCashInTransaction(
    ...
    const returnURL = slashAgnostic(
      ...
      '/v1/contract/modify-to-permanent?customerReference=',
    );
