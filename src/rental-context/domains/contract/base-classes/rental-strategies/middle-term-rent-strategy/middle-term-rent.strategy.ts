...
export class MiddleTermRentStrategy extends RentalStrategyBase {
  constructor({ contract }: { contract: ContractEntity }) {
    super(contract);
    this.logger = new Logger('MiddleTermRentStrategy');
  ...
  handle(): PaymentResponse {
    ...
    this.dateGuard.validateOrThrowError({ arrivalDate, departureDate }, RentPeriodStrategyType.MIDDLE_TERM_RENT);
    ...
    return {
      data: stayDates.map(({ startDate, endDate }, index) => {
        ...
        return {
          ...
          rentPeriodStrategyType: RentPeriodStrategyType.MIDDLE_TERM_RENT,
        };
      }),
    };
  }
