...
@Injectable()
export class EditApartmentAdDetailsService {
  constructor(
  ...
  async handle(dto: EditApartmentAdDetailsRequest, userId: UserOrmEntity['id']): Promise<Result<UUID, HttpException>> {
    const {
      id,
      numberOfGuests,
      numberOfRooms,
    ...
    apartmentAd.setCharacteristics({
      ...
      electricitySupply,
      objectPlacement,
      light: communications?.includes(CommunicationsEnum.LIGHT),
      water: communications?.includes(CommunicationsEnum.WATER),
      gas: communications?.includes(CommunicationsEnum.GAS),
      sewerage: communications?.includes(CommunicationsEnum.SEWERAGE),
      heating: communications?.includes(CommunicationsEnum.HEATING),
      ventilation: communications?.includes(CommunicationsEnum.VENTILATION),
      yearOfConstruction,
    });

    if (name) apartmentAd.setDescription({ name });
