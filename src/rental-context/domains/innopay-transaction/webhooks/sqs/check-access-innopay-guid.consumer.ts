...
@Injectable()
export class CheckAccessInnopayGuidConsumer {
  constructor(private readonly innopayTransactionBullsQueue: InnopayTransactionBullsQueue) {}

  @SqsMessageHandler(SQSQueues.checkAccessInnopayGuidQueue.name, false)
  async handle(queueMessage: AWS.SQS.Message) {
    ...
    if (!isSqsMessageTypeValid(body)) {
      ...
      throw new ArgumentInvalidException(`Invalid sqs message type for check access innopay guid`);
    }
